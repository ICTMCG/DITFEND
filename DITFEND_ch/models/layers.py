...
class Meta_Conv1d(torch.nn.Conv1d):
    def __init__(self, in_channels, out_channels, kernel_size):
        super(Meta_Conv1d, self).__init__(in_channels, out_channels, kernel_size)
        self.weight.fast = None
        self.bias.fast = None

    def forward(self, x):
        if self.weight.fast is not None and self.bias.fast is not None:
            out = self._conv_forward(x, self.weight.fast, self.bias.fast)
        else:
            out = super(Meta_Conv1d, self).forward(x)
        return out

class Meta_Embedding(torch.nn.Embedding):
    def __init__(self, num_embeddings, embedding_dim):
        super(Meta_Embedding, self).__init__(num_embeddings, embedding_dim)
        self.weight.fast = None
    ...
    def forward(self, x):
        if self.weight.fast is not None:
            ...
            out = super(Meta_Embedding, self).forward(x)
        return out

class MLP(torch.nn.Module):

    def __init__(self, input_dim, embed_dims, dropout, output_layer=True, BN=True):
        super().__init__()
        layers = list()
...
class cnn_extractor(nn.Module):
    def __init__(self, feature_kernel, input_size):
        ...
        self.convs = torch.nn.ModuleList(
            [Meta_Conv1d(input_size, feature_num, kernel)
...
class MaskAttention(torch.nn.Module):
    ...
    def __init__(self, input_shape):
        ...
        self.attention_layer = Meta_Linear(input_shape, 1)
...
class MultiHeadedAttention(torch.nn.Module):
    ...
    def __init__(self, h, d_model, dropout=0.1):
        ...
        self.linear_layers = torch.nn.ModuleList([Meta_Linear(d_model, d_model) for _ in range(3)])
        self.output_linear = Meta_Linear(d_model, d_model)
...
class SelfAttentionFeatureExtract(torch.nn.Module):
    def __init__(self, multi_head_num, input_size, output_size):
        ...
        self.out_layer = Meta_Linear(input_size, output_size)
