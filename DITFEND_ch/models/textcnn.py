...
class TextCNNModel(torch.nn.Module):
    ...
    def get_param(self):
        return list(self.convs.parameters()) + list(self.mlp.parameters())
    
    def get_weight_param(self):
        return list(self.weight_classifier.parameters())

    def forward(self, **kwargs):
        inputs = kwargs['content']
        masks = kwargs['content_masks']
        if self.emb_type == 'bert':
            emb_feature = self.bert(inputs, attention_mask = masks)[0]
        elif self.emb_type == 'w2v':
            emb_feature = inputs
        feature = self.convs(emb_feature)
...
class Trainer():
    def __init__(self,
        ...
        self.model = TextCNNModel(self.emb_dim, self.mlp_dims, self.task_num, self.bert_emb, self.dropout, self.emb_type)
        if self.use_cuda:
        ...
        for epoch in range(self.epoches):
            ...
            if mark == 'save':
                torch.save(self.model.state_dict(),
                    os.path.join(self.save_param_dir, 'parameter_textcnn_' + self.emb_type + '_' + str(self.task_num) + 'domains_finetune_' + str(finetune_domain) + '.pkl'))
            elif mark == 'esc':
                break
            else:
                continue
        if(self.epoches > 0):
            self.model.load_state_dict(torch.load(os.path.join(self.save_param_dir, 'parameter_textcnn_' + self.emb_type + '_' + str(self.task_num) + 'domains_finetune_' + str(finetune_domain) + '.pkl')))
        results = self.test(self.test_loader)
        pretty_results = get_pretty_result(results)
        with open(os.path.join(self.log_dir, 'textcnn_' + str(self.task_num) + '_finetune_' + str(finetune_domain) + '_results.txt'), 'w') as results_file:
            print(pretty_results, file = results_file)
        print(pretty_results)
        return pretty_results, os.path.join(self.save_param_dir, 'parameter_textcnn_' + self.emb_type + '_' + str(self.task_num) + 'domains_finetune_' + str(finetune_domain) + '.pkl')
